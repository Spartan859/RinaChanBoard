...
var touchstart = function (event, ownerInstance) {
  ...
  if (!st.size) return; // console.log('touchstart', JSON.stringify(event))
...
var touchmove = function (event, ownerInstance) {
  var ins = event.instance;
  var st = ins.getState();
  if (!st.size || !st.isMoving) return; // console.log('touchmove', JSON.stringify(event))

  var pagex = event.touches[0].pageX - st.startX;
  var pagey = event.touches[0].pageY - st.startY; // 左侧45度角为界限，大于45度则允许水平滑动
  ...
  var movex = pagex > 0 ? Math.min(st.max, pagex) : Math.max(-st.max, pagex); // 往回滑动的情况
...
var touchend = function (event, ownerInstance) {
  var ins = event.instance;
  var st = ins.getState();
  if (!st.size || !st.isMoving) return; // 左侧45度角为界限，大于45度则允许水平滑动
  ...
  st.isMoving = false; // console.log('touchend', JSON.stringify(event))

  var btns = ownerInstance.selectAllComponents('.btn');
  var len = btns.length;
  var i = len - 1; // console.log('len size', len)
...
var sizeReady = function (newVal, oldVal, ownerInstance, ins) {
  var st = ins.getState(); // st.disable = newVal && newVal.disable

  if (newVal && newVal.button && newVal.buttons) {
    st.size = newVal;
    st.transformx = 0; // var min = newVal.button.width
    ...
    for (; i >= 0; i--) {
      max += newVal.buttons[i].width; // if (min > newVal.buttons[i]) {
...
var showChange = function (newVal, oldVal, ownerInstance, ins) {
  var st = ins.getState();
  st.show = newVal;
  if (st.disable) return; // console.log('show change')
...
var rebounceChange = function (newVal, oldVal, ownerInstance, ins) {
  var st = ins.getState(); // console.log('rebounce', st.rebounce)
